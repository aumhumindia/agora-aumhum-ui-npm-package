...
function SelectMenuCamera() {
  ...
  // Listen for automatic device changes to update the UI
  useEffect(() => {
    const handleDeviceChange = (event: any) => {
      if (event.detail.automatic) {
        setCameraDeviceId(event.detail.deviceId)
        // Update available devices list
        AgoraRTC.getCameras().then(setAvailableCameraDevices)
      }
    }

    window.addEventListener('cameraDeviceChanged', handleDeviceChange)
    return () => window.removeEventListener('cameraDeviceChanged', handleDeviceChange)
  }, [])
  ...
  const handleChange = async (event: SelectChangeEvent) => {
    ...
    // Get device label for the hook
    const selectedDevice = availableCameraDevices.find(
      (c) => c.deviceId === newDeviceId
    )

    // Dispatch event with label information
...
function SelectMenuMic() {
  ...
  useEffect(() => {
    const handleDeviceChange = (event: any) => {
      if (event.detail.automatic) {
        setMicDeviceId(event.detail.deviceId)
        // Update available devices list
        AgoraRTC.getMicrophones().then(setAvailableMicrophoneDevices)
      }
    }

    window.addEventListener('microphoneDeviceChanged', handleDeviceChange)
    return () => window.removeEventListener('microphoneDeviceChanged', handleDeviceChange)
  }, [])
  ...
  const handleChange = async (event: SelectChangeEvent) => {
    ...
    // Get device label for the hook
    const selectedDevice = availableMicrophoneDevices.find(
      (m) => m.deviceId === newDeviceId
    )
...
function SelectMenuPlayback() {
  ...
  useEffect(() => {
    const handleDeviceChange = (event: any) => {
      if (event.detail.automatic) {
        setPlaybackDeviceId(event.detail.deviceId)
        // Update available devices list
        AgoraRTC.getPlaybackDevices().then(setAvailablePlaybackDevices).catch(console.log)
      }
    }

    window.addEventListener('playbackDeviceChanged', handleDeviceChange)
    return () => window.removeEventListener('playbackDeviceChanged', handleDeviceChange)
  }, [])
  ...
  const handleChange = async (event: SelectChangeEvent) => {
    ...
    // Get device label for the hook
    const selectedDevice = availablePlaybackDevices.find(
  ...
  return (
    <Box sx={{ marginTop: 5 }}>
      <FormControl fullWidth>
        <InputLabel id='playback-select-label'>
